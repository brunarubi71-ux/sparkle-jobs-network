  ...
  const acceptJob = async (job: Job) => {
    ...
    try {
      ...
      await supabase.from("profiles").update({
        jobs_used_today: currentUsed + 1,
        jobs_used_date: today,
      }).eq("id", user.id);
      ...
      toast.success("Job accepted! Chat opened.");
    ...
    <div className="min-h-screen bg-background pb-20">
      ...
      <div className="h-48 gradient-primary relative flex items-end justify-center pb-4">
        ...
        {filtered.slice(0, 3).map((job, i) => (
          <div
            key={job.id}
            className="absolute bg-card text-foreground text-xs font-bold px-2 py-1 rounded-full shadow-card"
      ...
      <div className="px-4 -mt-5 relative z-10">
        <div className="bg-card rounded-2xl shadow-card flex items-center px-4 gap-2">
          ...
          <Input
            ...
            className="border-0 bg-transparent h-12 focus-visible:ring-0 pl-0"
          />
          <button onClick={() => setShowFilters(!showFilters)} className="text-muted-foreground hover:text-primary">
          ...
          filtered.map((job, i) => (
            ...
            >
              <div className="flex items-start justify-between mb-2">
                ...
                <Badge className={`${urgencyColor(job.urgency)} border-0 text-[10px]`}>
                  {job.urgency.toUpperCase()}
                </Badge>
              </div>
              <div className="flex items-center gap-3 text-xs text-muted-foreground mb-3">
                <span className="flex items-center gap-1">
                  <MapPin className="w-3 h-3" /> {job.city || "N/A"}
                </span>
                <span className="flex items-center gap-1">
                  <Bed className="w-3 h-3" /> {job.bedrooms}
                </span>
                <span className="flex items-center gap-1">
                  <Bath className="w-3 h-3" /> {job.bathrooms}
                </span>
      ...
      <PremiumModal
        ...
        message="You've reached your daily limit of 2 jobs. Start your 7-day free trial to unlock unlimited jobs."
      />
      <BottomNav />
    </div>
  );
