...
export default function Schedules() {
  ...
  const unlockContact = async (scheduleId: string) => {
    ...
    await supabase.from("profiles").update({
      free_contacts_used: profile.free_contacts_used + 1,
    }).eq("id", user.id);
    setUnlockedIds((s) => new Set(s).add(scheduleId));
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      <div className="px-4 pt-6 pb-4">
        ...
        <p className="text-sm text-muted-foreground">Browse cleaning schedules for sale</p>
      </div>
            ...
            >
              <div className="flex justify-between items-start mb-3">
                <div>
                  ...
                  <p className="text-lg font-bold text-foreground">
                    {s.asking_price ? `$${s.asking_price}` : "Contact for price"}
                  </p>
                </div>
                <div className="text-right">
                  <div className="flex items-center gap-1 text-xs text-muted-foreground">
                    <Home className="w-3 h-3" /> {s.number_of_houses} houses
                  </div>
              ...
              {s.monthly_income_estimate && (
                <div className="flex items-center gap-1 text-sm text-primary font-medium mb-2">
                  <DollarSign className="w-3.5 h-3.5" /> ${s.monthly_income_estimate}/mo income
                </div>
              )}
              ...
              {isUnlocked(s.id) ? (
                ...
                >
                  ...
                  {canUnlockContact() ? "Unlock Contact" : "Unlock with Premium"}
                </Button>
              )}
            </motion.div>
          ))
      ...
      <PremiumModal
        ...
        message="You've used your 1 free contact unlock. Go Premium for unlimited access."
      />
      <BottomNav />
    </div>
  );
