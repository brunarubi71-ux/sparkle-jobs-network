    ...
    if (data) {
      ...
      if (cleanerId) {
        const [{ data: cp }, { data: revs }] = await Promise.all([
          (supabase as any).from("public_profiles").select("id, full_name, avatar_url").eq("id", cleanerId).maybeSingle(),
        ...
        <AnimatePresence>
          {showShare && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -8 }}
              className="bg-card rounded-2xl shadow-card p-4 flex gap-3"
            >
              <button
                onClick={() => {
              ...
              <button
                onClick={() => {
                  const url = encodeURIComponent("https://shinely.app");
                  window.open(`https://www.facebook.com/sharer/sharer.php?u=${url}`, "_blank");
                }}
                className="flex-1 flex items-center justify-center gap-2 bg-[#1877F2] hover:bg-[#0d6de0] text-white font-semibold text-sm rounded-xl py-3 transition-colors"
              >
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
