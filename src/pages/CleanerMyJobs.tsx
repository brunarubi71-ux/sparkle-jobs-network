...
const statusConfig: Record<string, { color: string; label: string; icon: string }> = {
  hired: { color: "bg-amber-100 text-amber-700", label: "ACCEPTED", icon: "🤝" },
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      <div className="px-4 pt-6 pb-4">
        ...
        <p className="text-sm text-muted-foreground">Track your accepted jobs</p>
      </div>
            ...
            {loading ? Array.from({ length: 3 }).map((_, i) => <ShimmerCard key={i} />) :
              activeJobs.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">Accept a job to get started!</p>
                </div>
